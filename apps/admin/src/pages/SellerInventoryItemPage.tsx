...
export function SellerInventoryItemPage() {
  const { productId } = useParams<{ productId: string }>();
  const [item, setItem] = useState<SellerStockItemView | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const res = await getSellerStockItem(productId!);
        if (cancelled) return;
        setItem(res);
        setNotFound(false);
        setLoadError(null);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setFormError(null);
    try {
      await createSellerMovement(productId, { type, quantity: qty, reason: reason.trim() });
      setQuantity('');
      setReason('');
