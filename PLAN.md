...
**Admin auth 🟡 in progress** (branch `feat/admin-auth-shell`; spec + plan in `docs/superpowers/specs|plans/2026-06-16-admin-auth-shell*`). Executing the 14-task plan one task at a time. **Done so far:** Task 1 — API CORS enabled for `:5001`/`:5002` (`apps/api/src/main.ts`); Task 2 — seeded `admin@example.com` (ADMIN) + `inventory@example.com` (INVENTORY_MANAGER) dev users, password `Password123!`, into `ecom_dev` (idempotent upsert in `apps/api/prisma/seed.ts`). Task 3 — installed `react-router-dom` v7 + added `VITE_API_URL` config (`apps/admin/src/lib/config.ts`, typed via `vite-env.d.ts`, trailing-slash-trimmed) + `.env.example`/`.env.local`. Task 4 — shared admin auth types (`apps/admin/src/lib/types.ts`: `Role`, `AuthUser`, `TokenPair`, `SessionExpiredError`, `ApiError`; `ApiError` uses an explicit `status` field since the admin tsconfig bans TS parameter properties via `erasableSyntaxOnly`). Confirmed `AuthUser.sub` matches the API `/auth/me` payload key. Task 5 — `tokenStore` over localStorage (TDD, key `admin.auth`; defensively treats partial/corrupt/unparseable storage as empty + clears it; 5 tests green). Task 6 — `apiClient` (TDD): Bearer-attaching `fetch` wrapper; on 401-with-token refreshes once via a module-level shared in-flight promise (concurrent 401s spend the rotating refresh token only once), retries with the rotated token; refresh-fail clears store + throws `SessionExpiredError`; other non-OK → `ApiError`; 204 → undefined. Contracts JSDoc'd on `request()`. 5 tests green (full admin suite 11/11). Task 7 — `isInternalRole` helper (TDD; `ReadonlySet<Role>` allow-list — ADMIN/INVENTORY_MANAGER → true, CUSTOMER → false; new roles default to rejected; 3 tests green). Task 8 — `AuthProvider` + `useAuth` (TDD): React Context owning auth state; boots from `GET /auth/me` (sole identity authority — no client-side JWT decode); `{ status: loading|authed|guest, user, login, logout }`; StrictMode-safe cancelled-flag boot; `login` POSTs login→me, `logout` best-effort revoke then always clears. Provider+hook colocated with one scoped `react-refresh/only-export-components` disable (ratified). 3 boot tests green (full suite 17/17). Task 9 — presentational components: `AppShell` (sidebar + header with SR-labelled user email + logout, `<Outlet/>`), `DashboardPage`, `AccessDeniedPage`, `LogoutButton`. DESIGN.md tokens only; a11y-hardened (focus-visible rings w/ offset, `<nav aria-label="Sidebar">`, `aria-current="page"`, semantic landmarks). No tests this task (exercised by Tasks 10–12); suite still 17/17. Task 10 — `ProtectedRoute` role gate (TDD): `loading`→accessible status indicator; `guest`→`<Navigate to="/login" replace>`; CUSTOMER→`AccessDeniedPage`; ADMIN/INVENTORY_MANAGER→`<Outlet/>`. Loading-flash guard test (no protected content before auth resolves) + UX-only-gate comment (API is the real authority). 5 tests green (full suite 22/22). Task 11 — accessible `LoginPage` (TDD): email/password form → `useAuth().login()`; success→navigate `/` (replace); 401→generic "Invalid email or password." / else→"Something went wrong." in `role="alert"`, focus moved to it via `useEffect([error])`; submit disabled while pending; `aria-invalid`+`aria-describedby` wiring; generic errors avoid account-existence leak. 3 tests green (full suite 25/25). Task 12 — assembled the app: `router.tsx` (`createBrowserRouter`: `/login` public → LoginPage; protected `ProtectedRoute` → `AppShell` → index `DashboardPage`; `*` → redirect `/`); `main.tsx` mounts `RouterProvider` inside `AuthProvider`; deleted the Vite scaffold (`App.tsx`/`App.test.tsx`/`App.css`) + AppShell behavior test. **Admin app now runnable & fully wired**; full suite 26/26, build clean. **Next:** Task 13 — manual smoke vs `ecom_dev` (real login end-to-end). (Notes: Task 13 needs a CUSTOMER account — none seeded yet; consider `eslint-plugin-jsx-a11y` + nav `<a>`/`NavLink` conversion in a later phase.)
