...
| M3 | Catalog V2 + Inventory V2 + Search (parallel group) | L | 🟡 In Progress — **M3a Slice 1 (public seller-read API) done** (branch `feat/catalog-v2`, not yet merged): public `GET /sellers/:slug` (5-field public view, leak-guarded) + `GET /sellers/:slug/products` (ACTIVE-only, reuses `ProductsService.list` via orthogonal `{sellerId}` filter); ACTIVE-seller gate → 404 otherwise on both; 380 unit + e2e green, tsc/lint clean, HTTP-smoked vs `ecom_dev`; per-task + whole-branch (opus) reviewed READY. Remaining M3a: storefront "sold by" link (Slice 2), `/seller/[slug]` page (Slice 3), `ratingAvg/ratingCount` cols + placeholders (Slice 4 / migration F2). M3b/M3c not started. |
