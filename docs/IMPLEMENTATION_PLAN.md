...
| M2 | Seller System | L | 🟡 (slices 1–3 done & smoke-verified vs `ecom_dev`: 1 ownership migration B1–B5 + call-site fixes; 2 service-layer ownership scoping — `buildSellerScope` + products/inventory scoped, cross-tenant 404, admin unchanged; 3 seller product CRUD API at `/seller/products` + `SellerApprovedGuard` attaches `sellerId` + `@CurrentSeller`, seller isolation proven over HTTP (e2e: own CRUD, cross-tenant 404, non-seller 403); 4 CSV bulk import `POST /seller/products/import` — multipart upload, size+row caps, per-row validation, partial-success report, products seller-owned (file `sellerId` column ignored — e2e-proven); 5 seller inventory API `/seller/inventory` — scoped stock list/detail/movements (cross-tenant 404, non-seller 403, e2e-proven), low-stock alert now also notifies the owning seller. **M2 backend complete (slices 1–5).** Slice 6 (admin seller-portal UI) in progress: **6a foundation done** — SELLER admitted to the shell (`canEnterShell`), `SellerOnlyRoute`, role-branched `AppShell` nav + role-branched `IndexDashboard`, seller dashboard (honest metrics), seller API clients (`/seller/*`), seeded demo seller (`seller@example.com`); 190 admin tests, runtime-smoked. **6b My Products done** — seller products list (pagination + status actions) + create/edit forms at `/seller/products` (reuse `ProductForm`/`StatusBadge`/`Pagination`/`RowActionsMenu`, seller-scoped client); 210 admin tests; runtime-smoked. **6c CSV upload done** — `SellerProductImportPage` (file picker + per-row result report) at `/seller/products/import`, `apiClient` FormData fix, `importSellerProducts` client; 217 admin tests; runtime-smoked. **6d My Inventory done** — `/seller/inventory` stock list (low-stock filter + pagination) + detail/movement-history/adjust (reuse `LowStockBadge`/`Pagination` + `lib/inventory` movement types); `SellerComingSoon` placeholder removed; 233 admin tests; runtime-smoked (list/detail/movement +5/low-stock filter). **Seller portal (6a–6d) complete.** Remaining: 6e admin "sold by") |
