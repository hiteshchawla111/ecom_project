...
| M3 | Catalog V2 + Inventory V2 + Search (parallel group) | L | 🟡 In Progress — **M3a Slices 1–3 done** (branch `feat/catalog-v2`, not yet merged). **Slice 1 (public seller-read API):** `GET /sellers/:slug` (5-field public view, leak-guarded) + `GET /sellers/:slug/products` (ACTIVE-only, reuses `ProductsService.list` via orthogonal `{sellerId}` filter); ACTIVE-seller gate → 404 otherwise; HTTP-smoked. **Slice 2 (storefront "sold by" link):** storefront `Product.seller?` type + server-compatible `SellerLink` ("Sold by &lt;name&gt;", name-only link to `/seller/[slug]`, `aria-label`, graceful omit) on product detail; SSR-smoked. **Slice 3 (`/seller/[slug]` page):** storefront `Seller` type + `getSeller`/`listSellerProducts` catalog client (404→null, reuse `toQuery`/`Paginated`) + the seller storefront page (profile header logo/name/description/count + product grid + `Pagination` + profile-first `notFound`), mirroring the category-detail page; SSR-smoked vs `ecom_dev` (`/seller/demo-shop` 200 with heading + count + product links; `/seller/no-such-shop` 404; Slice-2 "Sold by" link now round-trips to 200). All 3 slices per-task + whole-slice (opus) reviewed READY; 207 storefront tests + 380 api green. Remaining M3a: `ratingAvg/ratingCount` cols + placeholders (Slice 4 / migration F2). M3b/M3c not started. Follow-up (pre-existing): storefront `api-auth.test.ts:21` TS7022 stray tsc error (Vitest green) — track for M7d. |
