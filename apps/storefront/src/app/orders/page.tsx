...
export default async function OrdersHistoryPage() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-4 py-10">
      <header>
        <h1 className="text-2xl font-semibold text-content">Your orders</h1>
      </header>

      {history.data.length === 0 ? (
        <div className="flex flex-col items-start gap-3 rounded-lg border border-line p-8">
          <p className="text-content-muted">You haven&apos;t placed any orders yet.</p>
          <Link
            href="/products"
          ...
          {history.data.map((order) => (
            <li key={order.id}>
              <Link
                ...
                className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-line p-4 transition-colors hover:border-line hover:bg-surface-sunk focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-700"
              >
