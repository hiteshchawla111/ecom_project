...
| M3 | Catalog V2 + Inventory V2 + Search (parallel group) | L | 🟡 In Progress — **M3a Slices 1–2 done** (branch `feat/catalog-v2`, not yet merged). **Slice 1 (public seller-read API):** `GET /sellers/:slug` (5-field public view, leak-guarded) + `GET /sellers/:slug/products` (ACTIVE-only, reuses `ProductsService.list` via orthogonal `{sellerId}` filter); ACTIVE-seller gate → 404 otherwise; HTTP-smoked. **Slice 2 (storefront "sold by" link):** storefront `Product.seller?` type + server-compatible `SellerLink` component ("Sold by &lt;name&gt;", name-only link to `/seller/[slug]`, `aria-label`, graceful omit, DESIGN.md tokens) rendered on product detail; SSR-smoked vs `ecom_dev` (link + href + aria-label render; `/seller/<slug>` 404s pre-Slice-3 as expected). Both slices per-task + whole-slice (opus) reviewed READY; 202 storefront tests + 380 api green. Remaining M3a: `/seller/[slug]` page (Slice 3), `ratingAvg/ratingCount` cols + placeholders (Slice 4 / migration F2). M3b/M3c not started. Follow-up (pre-existing): storefront `api-auth.test.ts:21` TS7022 stray tsc error (Vitest green) — track for M7d. |
